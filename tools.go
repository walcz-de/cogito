...
// upstream/cloud-proxy defect: some providers (via the cloud-proxy "translate" layer that maps
// Anthropic/Gemini tool-call streaming into OpenAI deltas) emit the full arguments object more
// than once, so the accumulated string is a concatenation "{...}{...}". A plain json.Unmarshal
// then fails with "invalid character '{' after top-level value". On that failure we recover the
// FIRST complete top-level JSON object via json.Decoder — this makes frontier-driven tool loops
// work while local llama.cpp models (correct incremental deltas) take the fast path unchanged.
